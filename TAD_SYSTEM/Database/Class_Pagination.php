...
class Pagination{
    ...
    function html_accounts(){   
        ...
        if ($this->_config['total_record'] > $this->_config['limit']) {
            $p = '<center class="pb-3 pt-1"><div class="margin-bottom-10"><div class="btn-group">';
            if ($this->_config['current_page'] > 0) {
                $p .= '<button type="button" class="btn btn-primary" onclick="page = 1;load_account()"><i class="fa fa-angle-double-left" aria-hidden="true"></i></button>';
            }
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++) {
                if ($this->_config['current_page'] == $i) {
                    $p .= '<button type="button" class="btn btn-primary active" onclick="page='.$i.';load_account()">'.$i.'</button>';
                } else {
                    $p .= '<button type="button" class="btn btn-primary" onclick="page='.$i.';load_account()">'.$i.'</button>';
                }
            }
            if ($this->_config['current_page'] > 0) {
                $p .= '<button type="button" class="btn btn-primary" onclick="page='.$this->_config['total_page'].';load_account()"><i class="fa fa-angle-double-right" aria-hidden="true"></i></button>';
            }
            $p .= '</div></div></center>';
        }
        return $p;
