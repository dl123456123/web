            ...
            <table class="table-auto w-full scrolling-touch min-w-850">
                <thead>
                    <tr class="v-border-hr select-none border-b-2 border-gray-300">
                           ...
                           <th class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1">2Fa / Đăng Kí</th>
...
$result = mysqli_query($tad->tad_db(), "SELECT * FROM history_ff WHERE nguoimua = '$username' ORDER BY id DESC ");
...
?>
                       ...
                       <td class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1"><?=$row['2fa']?>/<?=$row['dangky']?></td>
                       ...
                       <td class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1"><?=number_format($row['giatien'])?> <sup>VNĐ</sup></td>
