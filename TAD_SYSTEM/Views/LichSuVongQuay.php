            ...
            <table class="table-auto w-full scrolling-touch min-w-850">
                <thead>
                    <tr class="v-border-hr select-none border-b-2 border-gray-300">
                           ...
                           <th class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1">Phần Thưởng</th>
...
$result = mysqli_query($tad->tad_db(), "SELECT * FROM user_history_system WHERE username = '$username' AND action = 'Vòng Quay FreeFire' ORDER BY id DESC LIMIT 15");
while($row = mysqli_fetch_assoc($result)){
$stt++;
?>
                                                    <tr>
                       <th class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1">#<?=$stt?></th>
                       <td class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1"><?=$row['mota']?> /</td>
                       <td class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1"><span class="bg-green-600 text-white h-6 px-2">Đã Quay</span></td>
                       <td class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1"><?=$row['sotien']?> <sup>VNĐ</sup></td>
                       <td class="v-table-title py-2 text-sm font-bold text-gray-800 text-left px-1"><?php echo date('d/m - H:i:s',$row['time']);?></td>
                    </tr> 
                                      <?php } ?> 
                                                </tbody>
            </table>
